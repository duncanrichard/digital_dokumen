              ...
              <div class="modal fade" id="editModal-{{ $row->id }}" tabindex="-1" aria-labelledby="editModalLabel-{{ $row->id }}" aria-hidden="true">
                <div class="modal-dialog modal-lg modal-dialog-centered">
                  <div class="modal-content shadow-lg rounded-3">
                    ...
                    {{-- UPDATE FORM (wrap header+body+footer) --}}
                    <form method="post" action="{{ route('master.jenis-dokumen.update', $row->id) }}" id="updateForm-{{ $row->id }}">
                      ...
                      <div class="modal-body pt-0">
                        ...
                        <div class="mb-3">
                          ...
                          <input type="text" name="kode" value="{{ old('kode', $row->kode) }}" class="form-control" required>
                        </div>
                        ...
                        <div class="mb-3">
                          ...
                          <textarea name="deskripsi" rows="3" class="form-control">{{ old('deskripsi', $row->deskripsi) }}</textarea>
                        </div>
                      ...
                      <div class="modal-footer border-0 d-flex justify-content-between">
                        {{-- DELETE button submits external delete form via HTML5 "form" attribute (no nested forms) --}}
                        <button type="submit"
    ...
    <div class="modal-content shadow-lg rounded-3">
      <form method="post" action="{{ route('master.jenis-dokumen.store') }}">
        ...
        <div class="modal-body pt-0">
          ...
          <div class="mb-3">
            <label class="form-label required">Code</label>
            <input type="text" name="kode" value="{{ old('kode') }}" class="form-control @error('kode') is-invalid @enderror" required placeholder="e.g., SK, IN, OUT">
            @error('kode')<div class="invalid-feedback">{{ $message }}</div>@enderror
          </div>

          <div class="mb-3">
            <label class="form-label required">Name</label>
            <input type="text" name="nama" value="{{ old('nama') }}" class="form-control @error('nama') is-invalid @enderror" required placeholder="e.g., Surat Keputusan">
            @error('nama')<div class="invalid-feedback">{{ $message }}</div>@enderror
          </div>

          <div class="mb-3">
            <label class="form-label">Description</label>
            <textarea name="deskripsi" rows="3" class="form-control @error('deskripsi') is-invalid @enderror" placeholder="Short description...">{{ old('deskripsi') }}</textarea>
